...
const CompetitionCard = ({
  ...
  const getPlacementStyle = (placement: string) => {
    ...
    if (placement.includes("2nd"))
      return "bg-slate-300/20 text-slate-300 border-slate-400/50";
  ...
  return (
    <Card className="bg-card border-2 border-border hover:border-primary transition-all duration-300 overflow-hidden group hover:glow-red">
      ...
      <div className="p-6">
        <span
          className={`inline-block text-lg font-bold px-3 py-1 rounded border ${getPlacementStyle(placement)}`}
        >
          {placement}
        </span>
